    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', padding: '8px' }}>
      <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
        <input type="date" value={localDate}
          ...
          onBlur={() => onUpdate(pid, wid, exp.id, 'date', localDate, field)}
          style={{ ...inp({ flex: 1 }), fontSize: '11px' }} />
        <input type="number" inputMode="decimal" value={localAmount}
          onChange={e => setLocalAmount(e.target.value)}
          onBlur={() => onUpdate(pid, wid, exp.id, 'amount', localAmount, field)}
          placeholder="$0"
          style={{ ...inp({ width: '70px' }), color, fontSize: '12px' }} />
        <button onClick={() => onDelete(pid, wid, exp.id)}
          style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.2)', cursor: 'pointer', fontSize: '14px', padding: '0 4px' }}>✕</button>
      </div>
      <input type="text" value={localNote}
        onChange={e => setLocalNote(e.target.value)}
        onBlur={() => onUpdate(pid, wid, exp.id, 'note', localNote, field)}
        placeholder="Note (e.g. gas fee bridge)"
        style={{ ...inp({ width: '100%' }), fontSize: '11px' }} />
    </div>
  );
}
...
export default function Dashboard({ onReset }) {
  ...
  const MobileWalletCard = ({ w, proj, wIndex }) => {
    ...
    const filtered = allFeeds.filter(f =>
      f.symbol.toLowerCase().includes(localSearch.toLowerCase().replace('/usd', ''))
    ).slice(0, 15);
    ...
    return (
      <div style={{ background: BG3, border: `1px solid ${BORDER}`, borderRadius: '10px', overflow: 'visible', marginBottom: '8px' }}>
        ...
        <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              {w.walletType && (
                <span style={{ fontSize: '9px', fontFamily: 'monospace', padding: '2px 6px', borderRadius: '4px',
            ...
            {w.address && w.walletType && (
              <span style={{ fontSize: '9px', fontFamily: 'monospace', color: DIM }}>{w.address.slice(0, 6)}...{w.address.slice(-4)}</span>
            )}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ textAlign: 'right' }}>
              {(wc.spend > 0 || wc.income > 0 || wc.airdrop > 0) && (
                <div style={{ fontSize: '14px', fontFamily: 'monospace', fontWeight: 700, color: wc.netPnl >= 0 ? GREEN : RED }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', borderTop: `1px solid ${BORDER}`, borderBottom: isExpanded ? `1px solid ${BORDER}` : 'none' }}>
          {[
            { label: 'Holdings', val: wc.modalCost > 0 ? '$' + fmt(wc.modalNow) : '—', color: '#fff' },
            { label: 'Expenses', val: wc.spend > 0 ? '-$' + fmt(wc.spend) : '—', color: RED },
            { label: 'Income',   val: wc.income > 0 ? '+$' + fmt(wc.income) : '—', color: GREEN },
            { label: 'Airdrop',  val: wc.airdrop > 0 ? '$' + fmt(wc.airdrop) : '—', color: PURPLE },
          ].map(s => (
          ...
          <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <input
                ...
                onBlur={() => updW(proj.id, w.id, 'name', localName)}
                style={inp({ width: '100%' })} placeholder="Wallet name" />
              <input
                value={localAddr}
                onChange={e => setLocalAddr(e.target.value)}
                onBlur={() => handleAddressInput(proj.id, w.id, localAddr, wIndex + 1)}
                style={inp({ width: '100%', color: w.walletType === 'EVM' ? '#63b3ed' : w.walletType === 'SVM' ? PURPLE : DIM })}
                placeholder="Paste wallet address (EVM or SVM)" />
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <div style={{ position: 'relative', zIndex: 30 }}>
                <input
                  type="text"
                  value={localSearch}
                  onChange={e => { setLocalSearch(e.target.value); setShowDrop(true); }}
                  onFocus={() => setShowDrop(true)}
                  placeholder="Search asset (e.g. BTC, ETH, FOGO)"
                  style={inp({ width: '100%' })} />
                  ...
                  <div style={{ position: 'absolute', width: '100%', background: '#1a1a1a', border: `1px solid ${BORDER}`, borderRadius: '8px', maxHeight: '200px', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.6)', marginTop: '2px', zIndex: 50 }}>
                    {filtered.map(f => (
                      <button key={f.id}
                        onMouseDown={e => {
                          e.preventDefault();
                          setLocalSearch(f.display);
                          setShowDrop(false);
                          selectFeed(proj.id, w.id, f);
                        }}
                        style={{ width: '100%', textAlign: 'left', padding: '10px 12px', fontSize: '13px', fontFamily: 'monospace', color: MUTED, background: 'none', border: 'none', cursor: 'pointer', borderBottom: `1px solid rgba(255,255,255,0.04)` }}>
                        {f.display}
                      </button>
                    ))}
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                <div>
                  ...
                  <input
                    ...
                    onBlur={() => updW(proj.id, w.id, 'assetQty', localQty)}
                    placeholder="0.00"
                    style={inp({ width: '100%' })} />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onBlur={() => updW(proj.id, w.id, 'assetPrice', localPrice)}
                    placeholder={wc.live > 0 ? `$${fmt(wc.live)}` : '0.00'}
                    style={inp({ width: '100%' })} />
                </div>
              </div>
              ...
              {wc.modalCost > 0 && (
                <div style={{ fontSize: '11px', fontFamily: 'monospace', color: wc.unrealizedPnl >= 0 ? GREEN : RED }}>
                  Unrealized: {wc.unrealizedPnl >= 0 ? '+' : '-'}${fmt(Math.abs(wc.unrealizedPnl))} ({wc.unrealizedPct >= 0 ? '+' : ''}{fmt(wc.unrealizedPct)}%)
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {expenses.map(exp => (
                <ExpenseRow key={exp.id} exp={exp} pid={proj.id} wid={w.id} field="expenses" color={RED} onUpdate={updE} onDelete={delExpense} inp={inp} />
              ))}
              <button onClick={() => addExpense(proj.id, w.id)}
                style={{ fontSize: '12px', fontFamily: 'monospace', color: RED, background: 'rgba(252,129,129,0.05)', border: `1px dashed rgba(252,129,129,0.25)`, padding: '8px', borderRadius: '6px', cursor: 'pointer', textAlign: 'center' }}>
                + Add expense
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {incomes.map(inc => (
                <ExpenseRow key={inc.id} exp={inc} pid={proj.id} wid={w.id} field="incomes" color={GREEN} onUpdate={updE} onDelete={delIncome} inp={inp} />
              ))}
              <button onClick={() => addIncome(proj.id, w.id)}
                style={{ fontSize: '12px', fontFamily: 'monospace', color: GREEN, background: 'rgba(72,187,120,0.05)', border: `1px dashed rgba(72,187,120,0.25)`, padding: '8px', borderRadius: '6px', cursor: 'pointer', textAlign: 'center' }}>
                + Add income
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <input
                type="number" inputMode="decimal"
                value={localAirdrop}
                onChange={e => setLocalAirdrop(e.target.value)}
                onBlur={() => updW(proj.id, w.id, 'airdropValue', localAirdrop)}
                placeholder="0.00"
                style={inp({ width: '100%', color: PURPLE })} />
            </div>
                ...
                {AIRDROP_LABELS.map(label => {
                  ...
                  return (
                    <button key={label} onClick={() => toggleLabel(proj.id, w.id, label)}
                      style={{ fontSize: '12px', fontFamily: 'monospace', padding: '5px 14px', border: `1px solid ${active ? ls.border : BORDER}`, background: active ? ls.bg : 'transparent', color: active ? ls.color : DIM, cursor: 'pointer', borderRadius: '5px' }}>
                      {label}
                    </button>
                  );
                })}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '8px', borderTop: `1px solid ${BORDER}` }}>
              <select value={w.status || 'Active'} onChange={e => updW(proj.id, w.id, 'status', e.target.value)}
                style={{ fontSize: '12px', fontFamily: 'monospace', padding: '6px 12px', border: `1px solid ${st.border}`, background: st.bg, color: st.color, outline: 'none', cursor: 'pointer', borderRadius: '6px' }}>
                {STATUS_OPTIONS.map(s => <option key={s} style={{ background: '#1a1a1a', color: '#fff' }}>{s}</option>)}
              </select>
              <button onClick={() => delWallet(proj.id, w.id)}
                style={{ background: 'none', border: `1px solid rgba(252,129,129,0.2)`, color: RED, cursor: 'pointer', fontSize: '11px', fontFamily: 'monospace', padding: '6px 14px', borderRadius: '6px' }}>
                Delete
              </button>
            </div>
          </div>
    ...
    <div style={{ minHeight: '100vh', background: BG, color: '#fff', position: 'relative' }}>
      ...
      <nav style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '12px 16px' : '16px 28px', borderBottom: `1px solid ${BORDER}`, background: 'rgba(8,8,8,0.8)' }}>
        <button onClick={onReset} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
          ...
          <span style={{ fontWeight: 800, fontSize: isMobile ? '14px' : '15px', color: '#fff', letterSpacing: '0.02em' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '8px' : '16px' }}>
          ...
          <button onClick={onReset} style={{ fontSize: '11px', fontFamily: 'monospace', color: MUTED, background: 'rgba(255,255,255,0.05)', border: `1px solid ${BORDER}`, padding: '5px 12px', borderRadius: '7px', cursor: 'pointer' }}>
            Reset
          </button>
        </div>
      </nav>
            ...
            <div key={proj.id} style={{ background: BG2, border: `1px solid ${BORDER}`, borderRadius: '12px', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: isMobile ? '12px 14px' : '14px 18px', borderBottom: proj.open ? `1px solid ${BORDER}` : 'none', cursor: 'pointer', background: BG3 }}
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: isMobile ? '8px' : '20px', fontSize: '11px', fontFamily: 'monospace' }}>
                  {isMobile ? (
                    <span style={{ color: pc.totalNetPnl >= 0 ? GREEN : RED, fontWeight: 700 }}>
                      {pc.totalNetPnl >= 0 ? '+' : '-'}${fmt(Math.abs(pc.totalNetPnl))}
                    </span>
                  ) : (
                    <>
              ...
              {/* Wallets — mobile card view vs desktop table */}
              {proj.open && (
                isMobile ? (
                  <div style={{ padding: '12px' }}>
                    {proj.wallets.map((w, wIndex) => (
                      <MobileWalletCard key={w.id} w={w} proj={proj} wIndex={wIndex} />
                    ))}
                  ...
                  <div style={{ overflowX: 'auto' }}>
                              ...
                              <tr style={{ borderBottom: `1px solid rgba(255,255,255,0.04)`, verticalAlign: 'top' }}
                                ...
                                <td style={{ padding: '14px', minWidth: '180px' }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                    ...
                                    <input value={w.address || ''} onChange={e => handleAddressInput(proj.id, w.id, e.target.value, wIndex + 1)}
                                      placeholder="Paste wallet address..."
                                      style={{ ...inp({ width: '155px' }), fontSize: '10px', color: w.walletType === 'EVM' ? '#63b3ed' : w.walletType === 'SVM' ? '#a78bfa' : DIM }} />
                                    {w.walletType && (
                                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '9px', fontFamily: 'monospace', fontWeight: 700, letterSpacing: '0.1em', padding: '2px 8px', borderRadius: '4px', background: w.walletType === 'EVM' ? 'rgba(99,179,237,0.1)' : 'rgba(167,139,250,0.1)', border: `1px solid ${w.walletType === 'EVM' ? 'rgba(99,179,237,0.3)' : 'rgba(167,139,250,0.3)'}`, color: w.walletType === 'EVM' ? '#63b3ed' : '#a78bfa', width: 'fit-content' }}>
                                        <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: 'currentColor', display: 'inline-block' }}></span>
                                        {w.walletType}
                                      </div>
                                    )}
                                    {w.address && w.walletType && (
                                      <div style={{ fontSize: '9px', fontFamily: 'monospace', color: DIM }}>{w.address.slice(0, 6)}...{w.address.slice(-4)}</div>
                                    )}
                                  </div>
                                </td>
                                ...
                                <td style={{ padding: '14px' }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                    ...
                                    <button onClick={() => addExpense(proj.id, w.id)} style={{ fontSize: '10px', fontFamily: 'monospace', color: DIM, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', padding: 0 }} onMouseEnter={e => e.target.style.color = RED} onMouseLeave={e => e.target.style.color = DIM}>+ Add expense</button>
                                ...
                                <td style={{ padding: '14px' }}>
                                  <input type="number" value={w.airdropValue || ''} onChange={e => updW(proj.id, w.id, 'airdropValue', e.target.value)} placeholder="0" style={inp({ width: '100px', color: PURPLE })} />
                                </td>
                                ...
                                <td style={{ padding: '14px' }}>
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    {AIRDROP_LABELS.map(label => {
                                      ...
                                      return (
                                        <button key={label} onClick={() => toggleLabel(proj.id, w.id, label)}
                                          style={{ fontSize: '10px', fontFamily: 'monospace', padding: '3px 10px', border: `1px solid ${active ? ls.border : BORDER}`, background: active ? ls.bg : 'transparent', color: active ? ls.color : DIM, cursor: 'pointer', textAlign: 'left', borderRadius: '5px', transition: 'all 0.15s' }}>
                                          {label}
                                        </button>
                                      );
                                    })}
                    ...
                    <div style={{ padding: '10px 18px', borderTop: `1px solid ${BORDER}` }}>
                      <button onClick={() => addWallet(proj.id)} style={{ fontSize: '11px', fontFamily: 'monospace', color: DIM, background: 'none', border: 'none', cursor: 'pointer', letterSpacing: '0.05em' }} onMouseEnter={e => e.target.style.color = AMBER} onMouseLeave={e => e.target.style.color = DIM}>
                        + Add wallet
                      </button>
                    </div>
                  </div>
